...
<script>
    ...
    function userSearch(){
        ...
        $.post("/api/getUsersByName",
            ...
            function (dat, status) {
                if (dat.status == 0) {
                    $('#tab-user-list').empty();
                    $('#tab-user-list').append('<tr><th>姓名</th><th>操作</th></tr>');
    ...
    function carSearch(){
        ...
        $.post("/api/getCarByName",
            ...
            function (dat, status) {
                ...
                if (dat.status == 0) {
                    $('#tab-car-list').empty();
                    $('#tab-car-list').append('<tr><th>姓名</th><th>操作</th></tr>');
                    for (var i in dat.data.users) {
                        $('#tab-car-list').append(
                            '<tr>'+
                            '<td>'+ dat.data.users[i].name +'</td>'+
    ...
    require([
        ...
        // --------------------------路径分析-----------------------------------------
        //创建路径分析对象
        var routeAnalyst = new RouteTask("http://121.28.103.199:5567/arcgis/rest/services/outlets/network2/NAServer/路径");
        //创建路径参数对象
        ...
        //---------------------------地图检索开始-----------------------------------------
        // 实例化FindTask
        var findTask = new FindTask("http://121.28.103.199:5567/arcgis/rest/services/outlets/outlets2f/MapServer");
        // FindTask的参数
