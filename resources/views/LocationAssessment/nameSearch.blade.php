...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 检索查询 <span class="c-gray en">&gt;</span> 检索评估对象 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<script type="text/javascript">
    ...
    function gotoMap(uid,address) {
        console.log(uid,address);
        window.location.href = '/mapSelect?uid=' + uid + '&address=' + address;
    }
                ...
                if (dat.status == 0) {
                    ...
                    for (var i in dat.data) {
                        $('#search-list-table').append(
                            ...
                            '<td>' +
                            '<button class="btn btn-primary-outline radius size-MINI "' +
                            // 'onclick="gotoMap('+"'"+ dat.data[i].uid+ "'" +')">定位到地图 ' +
                            'onclick="gotoMap('+"'"+ dat.data[i].uid+ "'" +','+"'"+ dat.data[i].address+ "'" +')">定位到地图 ' +
                            ' </button> ' +
                            '</td>' +
                            '</tr>'
                        );
