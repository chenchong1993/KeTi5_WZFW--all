...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 检索查询 <span class="c-gray en">&gt;</span> 名称检索 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
                ...
                if (dat.status == 0) {
                    ...
                    for (var i in dat.data) {
                        $('#search-list-table').append(
                            ...
                            // 'onclick="show_set_pop(' + dat.data[i].id + ')">修改参数 ' +
                            // ' </button> ' +
                            // '<button class="btn btn-primary-outline radius size-MINI "' +
                            // 'onclick="delete_transponder(' + dat.data[i].id + ')">删除' +
                            // '</button>' +
                            // '</td>' +
                            '</tr>'
                        );
